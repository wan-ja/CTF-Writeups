...
### 7.2. 옵코드 직접 역산
IDA가 제시한 라벨을 따라가면 무한 루프에 빠지는 구조. `Options → General → Opcode bytes`를 활성화해 `EA [CA 10 40 00] [33 00]` 원본 바이트를 확인. `EA`는 JMPF 옵코드로 far jump임을 파악하고, 뒤따르는 오퍼랜드 바이트(`CA 10 40 00`)를 리틀 엔디언 규칙으로 역순 배열하여 실제 목적지 주소(`0x004010CA`)를 수동 도출.
...
### 7.5. Ghidra: 전체 64비트 강제 리로드 시 32비트 구간 오분석

바이너리 전체를 64비트(`x86:LE:64:default`)로 강제 리로드할 경우, Heaven's Gate로 전환된 구간뿐 아니라 원래 32비트인 구간까지 64비트 명령어 인코딩 규칙으로 잘못 해석됨을 확인.

strlen 인라인 루프(`0x401120`~`0x401125`) 구간에서, 원본 바이트 `41 84 c0 75 f9`가
* 64비트 해석: `TEST R8B,AL` (`0x41`을 REX 프리픽스로 소비) → 포인터 증가 명령어가 누락된 것처럼 보임
* 32비트 재해석: `INC ECX` (`0x41`을 독립된 1바이트 명령어로 처리) + `TEST AL,AL`

동일 바이트 시퀀스가 아키텍처 모드에 따라 완전히 다르게 해석되며, 해당 구간을 Ghidra에서 개별적으로 32비트로 재해석했을 때만 표준적인 strlen 패턴(포인터 순회 + NULL 체크 + 길이 계산)이 논리적으로 완성됨을 확인. 이를 통해 공격 수행 5번의 `CMP ECX, 0x20` 비교가 사용자 입력 문자열의 길이(32자) 검증이라는 근거를 명확히 확정.