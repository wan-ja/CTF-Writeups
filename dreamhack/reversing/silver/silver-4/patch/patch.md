...
> **참고: 스택 카나리(Stack Canary) 관련 코드**
> 디컴파일 코드에 반복 등장하는 `local_XX = DAT_140007010 ^ (ulonglong)스택변수` 및 `__stack_chk_fail` 패턴은 컴파일러가 자동 삽입하는 스택 카나리 보호 기법으로, 함수 진입 시 전역 시크릿 값과 스택 주소를 XOR해 저장해두고 함수 종료 직전 값 변조 여부를 검사해 스택 버퍼 오버플로우 공격을 탐지·차단하는 역할이며 본 취약점과는 직접적 관련이 없는 컴파일러 레벨 보호 코드임.

* **분석 결론:** 윈도우 API 특성상 `paint_pen_fun` 함수 호출을 개별적으로 무력화하기엔 횟수가 너무 많음. 따라서 해당 함수 메모리 진입점 자체를 `ret`으로 패치하여, 함수 호출 즉시 아무 동작 없이 반환되도록 만들어 가림막 생성 로직 원천 차단 필요. 해당 패치는 함수 진입 직후, `GdipCreatePen1` 호출(리소스 할당) 이전 시점에 적용되므로 펜 객체 자체가 생성되지 않아 `GdipDeletePen` 미호출로 인한 리소스 누수 문제가 발생하지 않음.
...
6. 해당 함수 자체를 무효화하기 위해 기드라 Listing 뷰에서 함수의 첫 번째 명령어 오프셋(`2B80`) 기록 및 주소 연산 준비. 함수 진입 직후, 매개변수 처리나 스택 카나리 세팅 등 어떤 부수 효과도 발생하기 전인 최상단 명령어에 패치를 적용해야, 스택 상태가 깨지지 않고 호출자(`paint_fun`)로 안전하게 복귀할 수 있음.

![패치 대상 오프셋 확보](./images/06-pen_addr.png)

7. x64dbg에 프로그램을 로드한 뒤, [기호] 탭에서 파일이 실제로 로드된 베이스 주소(`00007FF6DFE60000`) 획득. ASLR(Address Space Layout Randomization)로 인해 프로그램 실행 시마다 메모리에 로드되는 베이스 주소가 매번 랜덤하게 바뀌지만, 실행 파일 내부의 상대적 구조(오프셋)는 컴파일 시점에 고정되어 변하지 않음. 따라서 Ghidra가 표시하는 정적 주소(`0x140002b80`, 가상 베이스 `0x140000000` 기준)에서 오프셋(`0x2b80`)을 추출한 뒤, x64dbg에서 확인한 실제 로드 베이스 주소(`7FF6DFE60000`)에 더하여 실제 메모리상의 패치 대상 주소를 계산.
