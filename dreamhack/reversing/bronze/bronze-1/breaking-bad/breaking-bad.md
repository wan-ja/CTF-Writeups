...
gale = open("gale_notebook.txt", "rb").read()
gale = gale.replace(b"\r\n", b"\n").replace(b"\n", b"\r\n")  # LF -> CRLF 통일
positions = [i for i, b in enumerate(gale) if b == 32]
g_key_len = min(len(positions), 256)
g_key = bytes([((positions[i] & 0xFF) ^ (7 * i)) & 0xFF for i in range(g_key_len)])
