...
def flag():
    # ... (중략) ...
        ...
        # [!] 관리자 봇에게 'flag'라는 이름으로 진짜 FLAG 값을 쿠키로 심은 뒤
        #     /vuln?param=<입력값>에 실제로 접속시킴
...
* **분석 결론:** 사용자가 입력한 `param` 값이 이스케이프 처리 없이 그대로 반환되므로, 악성 스크립트를 삽입할 경우 브라우저에서 실행되는 **Reflected XSS** 취약점 존재. 이를 악용하여 내부 관리자 봇이 해당 URL을 열도록 유도한 뒤, /flag 라우트가 관리자 봇에게 flag라는 이름으로 진짜 FLAG 값을 쿠키로 부여한 뒤 /vuln?param=에 접속시키는 구조를 확인. /vuln의 XSS를 통해 document.cookie를 읽어 /memo로 전송시키면, 그 안에 담긴 flag 쿠키 값이 그대로 유출됨.
...
1. Burp Suite의 Repeater를 사용하여 `/flag` 엔드포인트로 `POST` 요청 전송. 파라미터(`param`)에 관리자 봇에게 심어진 flag 쿠키 값을 `/memo` 페이지로 전송시키는 XSS 페이로드 삽입.
...
4. 캡처된 응답 패킷의 HTML Body 내 `<pre>` 태그 영역에서 최종적으로 유출된 관리자 봇의 flag 쿠키(플래그) 식별.
...
* **네트워크 기반 탐지 룰 제안 (Snort):**
  ...
  - `alert tcp $EXTERNAL_NET any -> $HTTP_SERVERS $HTTP_PORTS (msg:"[Web] Reflected XSS - Cookie Theft Payload Detected"; flow:to_server,established; http_method; content:"POST"; http_uri; content:"/flag"; http_client_body; pcre:"/(%3C|<)script(%3E|>).*document\.cookie.*(%3C|<)\/script(%3E|>)/i"; sid:1000002; rev:1;)`